...
export const APP_VERSION = process.env.NEXT_PUBLIC_APP_VERSION || '1.0.14';
...
export const autoUpdateOnVersionChange = async () => {
    ...
    try {
        ...
        if (!isUpToDate) {
            // Check for prompt lock (don't spam the user)
            const hasPromptLock = document.cookie.includes('version_prompt_lock=true');

            if (hasPromptLock) {
                console.log('🔔 [Version] New version available (user already notified)');
                return;
            }

            console.log('🔔 [Version] New version detected. Prompting user...');

            // Set lock cookie (expires in 5 minutes - don't prompt again for a while)
            const date = new Date();
            date.setTime(date.getTime() + (5 * 60 * 1000));
            document.cookie = `version_prompt_lock=true; expires=${date.toUTCString()}; path=/`;

            // Show confirmation dialog (MANUAL reload)
            const shouldReload = window.confirm(
                '🚀 A new version of the app is available!\n\n' +
                'Click OK to refresh and get the latest updates.\n' +
                'Click Cancel to continue with the current version.'
            );
 ...
 */
export const startVersionCheck = (intervalMs = 30000) => { // Check every 30 seconds
    if (typeof window === 'undefined') return;
